...
MEAN_GRAY = 0.449 #GRAYSCALE
STD_GRAY = 0.226 #GRAYSCALE

#SHAPE CHANGE ((BATCH, HEIGHT, WIDTH, CHANNEL)) TO ((BATCH, CHANNEL, HEIGHT, WIDTH))
def to_tensor(image):
    image = np.ascontiguousarray(image.transpose(2, 0, 1))
    return torch.from_numpy(image).float()

#EITHER USE TO_IMAGE OR DENORMALIZE (DENORMALIZE DOES NOT ADD INCONSISTENCIES AFTER RECREATION)
def to_image(tensor, mean=MEAN, std=STD):
    ...
    image = denorm_tensor.numpy().astype(np.uint8)
    return image
...
def denormalize(image, mean=MEAN, std=STD):
    ...
    imag += mean
    imag *= 255.
    #imag.reshape((128,128,-1))
    return imag.astype(np.uint8)

#WIDTH/HEIGHT EXTENDER NO OBJECT DISTORTION DURING SCALING
class LetterBox:
...
#BASIC TRANSFORM ONLY LETTERBOX/RESIZE AND NORMALIZE
